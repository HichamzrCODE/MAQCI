...
class FsLigne {
    ...
    public function getAllByReleve($releve_id, $date_debut = null, $date_fin = null) {
        $query = "SELECT * FROM fs_lignes WHERE releve_id = ?";
        $params = [$releve_id];
        if ($date_debut) {
    ...
    public function create($releve_id, $date, $bc, $facture, $montant, $versement) {
        $this->db->prepare(
            "INSERT INTO fs_lignes (releve_id, date_operation, bc_fournisseur, numero_facture, montant, versement) VALUES (?, ?, ?, ?, ?, ?)"
        )->execute([$releve_id, $date, $bc, $facture, $montant, $versement]);
    }

    public function deleteByReleve($releve_id) {
        $this->db->prepare("DELETE FROM fs_lignes WHERE releve_id = ?")->execute([$releve_id]);
    }

    public function deleteByIds(array $ids) {
        if (empty($ids)) return;
        $in = implode(',', array_fill(0, count($ids), '?'));
        $stmt = $this->db->prepare("DELETE FROM fs_lignes WHERE id IN ($in)");
        $stmt->execute($ids);
    }
}